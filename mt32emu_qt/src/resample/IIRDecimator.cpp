...
IIRDecimator::IIRDecimator(const Quality quality) :
	IIRDecimator(0, NULL, NULL, quality)
{}

IIRDecimator::IIRDecimator(const unsigned int order, const IIRCoefficient numerator[], const IIRCoefficient denominator[]) :
	IIRDecimator(order, numerator, denominator, IIRDecimator::CUSTOM)
{}

IIRDecimator::IIRDecimator(const unsigned int order, const IIRCoefficient numerator[], const IIRCoefficient denominator[], const Quality quality) :
	c(order, numerator, denominator, quality),
...
void IIRDecimator::addInSamples(const FloatSample *&inSamples) {
	...
	for (unsigned int channel = 0; channel < IIR_DECIMATOR_CHANNEL_COUNT; ++channel) {
		BufferedSample sample = *(inSamples++);
		unsigned int delaySampleIx = ringBufferPosition + 1;
		for (unsigned int i = 1; i <= c.order; ++i) {
			sample -= c.denominator[i] * c.ringBuffer[(delaySampleIx++) & c.delayLineMask][channel];
		}
		c.ringBuffer[ringBufferPosition][channel] = sample;
	}
	--phase;
}
...
void IIRDecimator::getOutSamples(FloatSample *&outSamples) {
	for (unsigned int channel = 0; channel < IIR_DECIMATOR_CHANNEL_COUNT; ++channel) {
		...
		for (unsigned int i = 0; i <= c.order; ++i) {
			sample += c.numerator[i] * c.ringBuffer[(delaySampleIx++) & c.delayLineMask][channel];
		}
